...
def add_horse(name, horsehouse_id, load, hard_control, price):
    try:
        ...
        macs = (name, horsehouse_id, load, hard_control, price)
        cursor.execute("INSERT INTO horse (name, horsehouse_id, load, hard_control, price) VALUES (%s, %s, %s, %s, %s)", macs)
        conn.commit()  
...
def delete_horsehouse(id):
    try:
        ...
        cursor.execute("DELETE FROM horse WHERE id = (%s)", id)
        conn.commit()  
...
def watch_x_horsehouse_from_y(x, y):
    ...
    try:
        ...
        # macs = (0, "Max", 66, True, 105, 4,0)
        cursor.execute("SELECT * FROM horse GROUP BY id, weight LIMIT %s OFFSET %s", (y,x))
